...
module.exports = {
  ...
  banUser: async (req, res, next) => {
    try {
      ...
      // Try to find the user first in the Mentor collection
      let user = await Mentor.findById(id);

      // If not found in Mentor, try to find in the Student collection
      if (!user) {
        user = await Student.findById(id);
      }

      // If the user is still not found, return a not found response
      if (!user) {
  ...
  getAllInteractions: async (req, res, next) => {
    try {
      ...
      for await (const mentor of mentors) {
        const posts = await Post.find({ group_id: mentor._id }).populate(
          "author"
        );
